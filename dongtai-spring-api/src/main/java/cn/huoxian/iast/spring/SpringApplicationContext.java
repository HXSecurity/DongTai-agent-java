    ...
    public static List<ApiDataModel> getAPIList(WebApplicationContext applicationContext) {
        ...
        for (RequestMappingInfo info : methodMap.keySet()) {
            ...
            try {
                ...
                declaredMethod = AopUtils.getTargetClass(applicationContext.getBean(handlerMethod.getBean().toString()))
                ...
                for (Parameter parameter : parameters) {
                    ...
                    for (Annotation annotation : declaredAnnotations) {
                        String annoType = annotation.annotationType().toString();
                        String anno = annoType.substring(annoType.lastIndexOf(".") + 1);
                        switch (anno) {
                            case "PathVariable":
                                anno = "restful访问参数";
                                break;
