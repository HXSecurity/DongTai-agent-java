...
public class TaintRangeBuilderRemoveTest {
    ...
    public void testRemoveCharAt() {
        ...
        ts = new TaintRanges();
        tgtTs = new TaintRanges(new TaintRange(0, 7));
        // new StringBuilder("FOOZBAR").deleteCharAt(3)     // FOOBAR
        tb.remove(ts, new StringBuilder("FOOBAR"), tgtTs, 3, 0, 1);
        Assert.assertEquals("Taints:[untrusted(0,6)]", ts.toString());

        ts = new TaintRanges();
        tgtTs = new TaintRanges(new TaintRange(1, 6));
        // new StringBuilder("fOOZBAr").deleteCharAt(3)    // fOOBAr
        tb.remove(ts, new StringBuilder("fOOBAr"), tgtTs, 3, 0, 1);
        Assert.assertEquals("Taints:[untrusted(1,5)]", ts.toString());

        ts = new TaintRanges();
        tgtTs = new TaintRanges(new TaintRange(0, 3), new TaintRange(4, 7));
        // new StringBuilder("FOOzBAR").deleteCharAt(3)    // FOOBAR
        tb.remove(ts, new StringBuilder("FOOBAR"), tgtTs, 3, 0, 1);
        Assert.assertEquals("Taints:[untrusted(0,6)]", ts.toString());
    }
