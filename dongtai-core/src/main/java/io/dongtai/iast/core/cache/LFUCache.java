/*
  LFU缓存
  参考自ActiveMQ(org.apache.activemq.util.LFUCache),遵循Apache Licensed 2.0 协议:
  ...
  <p> http://www.apache.org/licen  ses/LICENSE-2.0 <p>
 ...
 */
package io.dongtai.iast.core.cache;
 ...
 * <ul>
 * <li>
 * Frequency list is stored as an array with no next/prev pointers between nodes: looping over the array should be faster and more CPU-cache
 * friendly than
 * using an ad-hoc linked-pointers structure.
 * </li>
 * <li>
 * The max frequency is capped at the cache size to avoid creating more and more frequency list entries, and all elements residing in the max
 * frequency entry
...
public class LFUCache<Key, Value> implements Map<Key, Value> {
    ...
    private void moveToNextFrequency(CacheNode<Key, Value> currentNode, int nextFrequency, LinkedHashSet<CacheNode<Key, Value>> currentNodes,
